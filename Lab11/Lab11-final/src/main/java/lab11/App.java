...
/**
 * Initializes and Maintains our app. 
 *
 *@author Violet Lempke, Jack Milos
 *
 */
public class App {
    static RoadMap nh;
    ...
    /**
     *Creates a new Graph Given a map.
     *
     * @return a new Graph
     */
    ...
    /**
     *Creates a graph with a highlighted path.
     *
     * @param coloredEdges the edges to be highlighted
     *
     * @return a new graph
     */
    static mxGraph getGraph(List<String> coloredEdges) {
        var nodes = new TreeMap<String, Object>();
        ...
        var step = 0;
        for (var name : coloredEdges) {
            var c0 = nodes.get(name);
            step += 1;
            if (step < coloredEdges.size()) {

                var c1 = nodes.get(coloredEdges.get(step));
                graph.insertEdge(top, null, "", c0, c1, "strokeColor=red;");
            }
    ...
    /**
     *Determines and Indicated the shortest path between two points. 
     *
    ...
    static void printPath(String src, String dst) {
        ...
        var path = (FindPath.findPath(nh, src, dst));
        var frame = new MapWindow(getGraph(path));
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setSize(700, 700);
        frame.setVisible(true);
...
/**
 *Determines behaviour and Detection for the Window. 
 *
 *@author Nat Tuck
 */
class MapWindow extends JFrame {
    ...
    /**
     *Determines behaviour for when mouse is clicked in window. 
     *
     *@param gcomp location of a cell
     *
     *@param ee if the mouse was clicked
     */
